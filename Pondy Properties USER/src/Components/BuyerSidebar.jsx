...
const BuyerSidebar = () => {
    ...
    const location = useLocation(); const storedPhoneNumber = location.state?.phoneNumber || localStorage.getItem("phoneNumber") || ""; const [phoneNumber, setPhoneNumber] = useState(storedPhoneNumber); const query = new URLSearchParams(useLocation().search); const ppcId = query.get('ppcId');
...
useEffect(() => {
    const fetchHelpOwnersCount = async () => {
        ...
        try {
            ...
            setHelpOwnersCount(data.helpPropertiesCount);   
        } catch (error) {
        }
    };
...
fetchContactBuyerCount();
 ...
 fontFamily: "Inter, sans-serif",
    ...
    backgroundColor: activeTab === tab ? '#30747F' : '#fff',
    ...
    <div className="d-flex align-items-center justify-content-start w-100"    style={{
       ...
           </button> <h3 className="m-0 ms-3" style={{fontSize:"20px"}}>Buyer Menu  </h3> </div>
