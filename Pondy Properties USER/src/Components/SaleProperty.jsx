...
const handleCardClick = async (ppcId, phoneNumber) => {
  ...
  if (!clickedProperties.includes(ppcId)) {
    const updated = [...clickedProperties, ppcId];
    setclickedProperties(updated);
    localStorage.setItem("clickedProperties", JSON.stringify(updated));
  }
  ...
  const appIntent = `intent://ppcpondy/${ppcId}#Intent;scheme=https;package=com.apps.ppcpondy;end`;

  const fallback = setTimeout(() => {
    window.location.href = "https://play.google.com/store/apps/details?id=com.apps.ppcpondy&hl=en";
  }, 1500);
