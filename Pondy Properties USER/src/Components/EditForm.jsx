...
function EditForm({ ppcId, phoneNumber }) {
    // State and Union Territory List
    const statesList = [
...
const handleClear = () => {
  ...
  const [removedPhotoUrls, setRemovedPhotoUrls] = useState([]); // Track removed photos
  const [removedVideoUrls, setRemovedVideoUrls] = useState([]); // Track removed videos
  ...
  useEffect(() => {
    const fetchPropertyData = async () => {
      try {
        ...
        // Store existing photos as URLs (not File objects)
        setPhotos(
        ...
        // Store existing videos
        setVideos(Array.isArray(data.video) ? data.video : []);

        // Reset removed media tracking on fresh load
        setRemovedPhotoUrls([]);
        setRemovedVideoUrls([]);
        ...
        // Initialize area input value
        setAreaInputValue(data.area || '');
      } catch (error) {
        console.error('Error fetching property data:', error);
        setMessage({
          text: 'Error loading property data. Please refresh.',
          type: 'error'
        });
      }
    };
...
// Image compression function to compress to 200KB
 ...
 const handlePhotoUpload = async (e) => {
  ...
  try {
          ...
          reader.onload = (event) => {
            ...
            img.onload = async () => {
              ...
              canvas.toBlob(async (blob) => {
                // Compress the watermarked image to 30KB with progress tracking
                setCompressionStatus("Compressing image...");
                const compressedBlob = await compressImageToSize(blob, 30, (progress) => {
   ...
   const removePhoto = (index) => {
    ...
    // If it's a string URL (existing photo from server), track it for deletion
    if (typeof photoToRemove === 'string') {
      setRemovedPhotoUrls([...removedPhotoUrls, photoToRemove]);
    }
...
// ⚡ Compress video using ffmpeg.wasm - Target 200KB
...
//   const updatedVideos = videos.filter((_, i) => i !== indexToRemove);
//   setVideos(updatedVideos);
// };

  const removeVideo = (indexToRemove) => {
    const videoToRemove = videos[indexToRemove];
    // If it's a string URL (existing video from server), track it for deletion
    if (typeof videoToRemove === 'string') {
      setRemovedVideoUrls([...removedVideoUrls, videoToRemove]);
    }
...
const shouldHideField = (fieldName) =>
 ...
 const handleAnim = () => {
  ...
  // Only append NEW photos (File objects), not existing URLs
  ...
  // Only append NEW videos (File objects), not existing URLs
  ...
  // Append removed photo URLs for deletion
  if (removedPhotoUrls.length > 0) {
    formDataToSend.append("removedPhotos", JSON.stringify(removedPhotoUrls));
  }

  // Append removed video URLs for deletion
  if (removedVideoUrls.length > 0) {
    formDataToSend.append("removedVideos", JSON.stringify(removedVideoUrls));
  }
  ...
  try {
    ...
    if (response.data.success || response.status === 200) {
      ...
      // Reset removed media tracking after successful submission
      setRemovedPhotoUrls([]);
      setRemovedVideoUrls([]);
      ...
      setMessage({
        text: response.data.message || "Error saving property data.",
        type: "error"
      });
    }
    ...
    let errorMessage = "Error saving property data.";
...
const renderDropdown = (field) => {
  ...
  // Get options list - use statesList for state field, otherwise use dataList
  let options = [];
  if (field === 'state') {
    options = statesList.map(s => s.name);
  } else {
    options = dataList[field] || [];
  }
          ...
          >
            ...
            >
              {filteredOptions.map((option, index) => {
                // Get the state type if this is the state field
                let displayLabel = option;
                let stateType = null;
                if (field === 'state') {
                  const stateObj = statesList.find(s => s.name.toLowerCase() === option.toLowerCase());
                  if (stateObj) {
                    stateType = stateObj.type;
                    displayLabel = stateObj.type === 'UT' ? `${stateObj.name} (UT)` : stateObj.name;
                  }
                }
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          if (el) el.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    ...
                    style={{
                      fontWeight: stateType === 'UT' ? 500 : 300,
                      padding: '8px 10px',
                      cursor: 'pointer',
                      color: stateType === 'UT' ? '#0B57CF' : 'grey',
                      marginBottom: '5px',
                      borderBottom: '1px solid #D0D7DE',
                      backgroundColor: stateType === 'UT' ? '#f0f7ff' : 'transparent',
                      borderRadius: '4px'
                    }}
                  >
                    {displayLabel}
...
{videos.length > 0 ? (
    ...
    <div className="d-flex flex-wrap gap-3">
      {videos.map((video, index) => {
        ...
        if (typeof video === "string") {
          videoUrl = `https://ppcpondy.com/PPC/${video}`;
          console.log(`Video [${index}] from backend:`, videoUrl);
        } else if ((video instanceof File || video instanceof Blob) && video.name) {
          videoUrl = `https://ppcpondy.com/PPC/${video.name}`;
          console.log(`Video [${index}] from uploaded file, using name:`, videoUrl);
        } else {
          videoUrl = URL.createObjectURL(video);
          console.log(`Video [${index}] fallback to preview:`, videoUrl);
        }
  ...
  <>
    ...
    <p className="text-muted mt-3">No video uploaded.</p>
  </>
)}
      ...
      >
        ...
        {statesList.map((stateObj, index) => (
          <option key={index} value={stateObj.name}>
            {stateObj.type === 'UT' ? `${stateObj.name} (UT)` : stateObj.name}
          </option>
        ))}
      </select>
...
<div className="form-group">
  ...
  {/* area */}
  <div className="form-group" ref={areaDropdownRef}>
  <label>Area:<span style={{ color: 'red', marginLeft: '4px' }}>*</span></label>
  <div className="input-card p-0 rounded-1" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%',  border: '1px solid #2F747F', background:"#fff", position: 'relative' }}>
    ...
    <input
      ...
      onFocus={() => {
        if (areaInputValue.length > 0) {
          const filtered = Object.keys(areaPincodeMap).filter(area =>
            area.toLowerCase().includes(areaInputValue.toLowerCase())
          );
          setFilteredAreas(filtered);
    ...
    {showAreaDropdown && filteredAreas.length > 0 && (
      <div style={{
        ...
        {filteredAreas.map((area, index) => (
          ...
          >
            <div style={{ fontSize: '13px', fontWeight: '500', color: '#2F747F' }}>
              {area}
            </div>
            <div style={{ fontSize: '12px', color: '#999', marginTop: '3px' }}>
              PIN: {areaPincodeMap[area]}
            </div>
          </div>
        ))}
      </div>
