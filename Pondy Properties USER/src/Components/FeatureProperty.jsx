    ...
    const fetchProperties = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL}/fetch-featured-properties-on-demand`
        );
...
const handleCardClick = (ppcId) => {
  if (!clickedPropert.includes(ppcId)) {
    const updatedClickedPropert = [...clickedPropert, ppcId];
...
onClick={() => handleCardClick(property.ppcId)}
  >
...
/>
         ...
         <div className="col-md-8 col-8 " style={{paddingLeft:"10px", paddingTop:"7px" , background: clickedPropert.includes(property.ppcId) ? "#ffffff" : "#F9F9F9",}}>
  ...
  : 'N/A'} 
        ...
        <p className="fw-bold m-0 " style={{ color:clickedPropert.includes(property.ppcId) ? "#F76F00" : "#000000", fontSize:"15px" }}>{property.propertyType 
