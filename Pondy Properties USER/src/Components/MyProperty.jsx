 ...
 <Tab.Content className="pt-3 w-100">
   ...
   >
                    ...
                    <div className="col-md-4 col-4 d-flex flex-column align-items-center">
                      <div className="text-white py-1 px-2 w-100 text-center" style={{ background: "#2F747F" }}>
                        PUC- {user.ppcId}
                      </div>
  ...
  src={
    user.photos && user.photos.length > 0
      ? `https://ppcpondy.com/PPC/${user.photos[0].replace(/\\/g, "/").replace(/^\/+/, "")}`
      : pic
  }
  alt={(
  ...
  <span style={{ fontSize:'13px', color:'#5E5E5E', fontWeight: 500 }}>
    {
      user.updatedAt && user.updatedAt !== user.createdAt
                           ...
                           : 'N/A'}
                                ...
                                <div className="col-12 d-flex align-items-center justify-content-between mt-2">
                                  <div style={{
                                    ...
                                    fontSize: '11px',
                                    fontWeight: '600',
                                    backgroundColor: user.status === 'complete' ? '#d4edda' : '#fff3cd',
                                    color: user.status === 'complete' ? '#155724' : '#856404'
                                  }}>
                                    {user.status === 'complete' ? '✓ Complete' : '⚠ Incomplete'}
                                  </div>
                                  <div style={{
                                    padding: '4px 10px',
                                    borderRadius: '12px',
                                    fontSize: '11px',
                                    fontWeight: '600',
                                    backgroundColor: user.payustatususer === 'paid' ? '#d4edda' : '#f8d7da',
                                    color: user.payustatususer === 'paid' ? '#155724' : '#721c24'
                                  }}>
                                    {user.payustatususer === 'paid' ? '✓ Paid' : 'Unpaid'}
                                  </div>
                                </div>
</div></div>
        ...
        onMouseOver={(e) => {
          e.target.style.background = "#629ecb"; // Brighter neon on hover
          e.target.style.fontWeight = 600; // Brighter neon on hover
          e.target.style.transition = "background 0.3s ease"; // Brighter neon on hover
        }}
        onMouseOut={(e) => {
          e.target.style.background = "#06aed5"; // Original orange
          e.target.style.fontWeight = 400; // Brighter neon on hover

        }}
                  onClick={(e) => {
        e.stopPropagation(); // prevent card click
        confirmDelete(user.ppcId);
      }}
           >Remove</button>
        ...
        className="btn btn-sm"
          ...
          width: '30%',
          transition: 'all 0.3s ease',
          border:"1px solid #38AFF1"
        }}
        onMouseOver={(e) => {
           e.target.style.color = "#fff";
          e.target.style.background = "#38AFF1"; // Brighter neon on hover
          e.target.style.fontWeight = 600; // Brighter neon on hover
          e.target.style.transition = "background 0.3s ease"; // Brighter neon on hover
        }}
        onMouseOut={(e) => {
          e.target.style.color = "#38AFF1"; 
          e.target.style.background = "white"; // Original orange
          e.target.style.fontWeight = 400; // Brighter neon on hover
}}
          onClick={(e) => {
        e.stopPropagation(); // prevent card click
        confirmEdit(user);
          ...
          >Edit</button>
                       ...
                       {user.status === "complete" && user.payustatususer !== "paid" && (
  <button
        className="btn btn-sm"
       style={{
            background: '#38AFF1',
           ...
           onMouseOver={(e) => {
            e.target.style.background = "#3882f1";
            e.target.style.fontWeight = 600;
            e.target.style.transition = "background 0.3s ease"; // Brighter neon on hover

          }}
          onMouseOut={(e) => {
            e.target.style.background = "#38AFF1";
  ...
  : 'N/A'}
     ...
     {user.price
          ...
          : 'N/A'}                 </span> 
               ...
               <div className="col-12 d-flex align-items-center justify-content-between mt-2 ps-1">
                 <div style={{
                   padding: '4px 10px',
                   borderRadius: '12px',
                   fontSize: '11px',
