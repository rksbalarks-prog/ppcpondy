...
useEffect(() => {
  const fetchProperties = async () => {
    ...
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/address-requests/owner/${phoneNumber}`);
      ...
      console.error("Address request fetch error:", error);
...
const handleRemoveProperty = async (ppcId) => {
  ...
  try {
    const response = await axios.put(`${process.env.REACT_APP_API_URL}/address-requests/delete/${ppcId}`);
    if (response.status === 200) {
      ...
      if (removedProperty) {
        const propertyWithOriginalStatus = {
          ...removedProperty,
          originalStatus: removedProperty.status  
        };
...
const handleUndoRemove = async (ppcId) => {
  ...
  try {
    const response = await axios.put(`${process.env.REACT_APP_API_URL}/address-requests/undo/${ppcId}`);
    if (response.status === 200) {
      ...
      if (restoredProperty) {
        ...
        const updatedRemoved = removedProperties.filter((p) => p.ppcId !== ppcId);
...
    </button> <h3 className="m-0 ms-3" style={{fontSize:"20px"}}>MY ADDRESS REQUESTS </h3> </div>
